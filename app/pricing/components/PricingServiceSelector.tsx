...
export default function PricingServiceSelector({
  ...
  return (
    <div className="relative z-30 mb-14 flex justify-center sm:mb-16" data-aos="fade-up">
      <LayoutGroup id="pricing-tabs">
        <div
          role="tablist"
          aria-label="Pricing category"
          className="relative z-30 inline-flex w-full max-w-3xl flex-col gap-1 rounded-2xl border border-neutral-200/80 bg-white p-1.5 shadow-[0_16px_40px_-24px_rgba(0,0,0,0.22)] sm:max-w-4xl sm:flex-row sm:rounded-full"
        >
          {services.map(({ id, icon: Icon, description }) => {
            ...
            return (
              <button
                ...
                onClick={() => onSelect(id)}
                className={cn(
                  "relative flex flex-1 items-center justify-center gap-2.5 rounded-xl px-4 py-3 text-left transition-colors sm:rounded-full sm:px-8 sm:py-3 sm:text-center",
              ...
              >
                {isActive ? (
                  <motion.span
                    ...
                    className="absolute inset-0 rounded-xl bg-neutral-900 shadow-md shadow-neutral-900/20 sm:rounded-full"
                ...
                <span className="relative z-10 flex min-w-0 flex-col sm:items-center">
                  ...
                  <span
                    className={cn(
                      "hidden text-[11px] font-normal leading-tight sm:block",
